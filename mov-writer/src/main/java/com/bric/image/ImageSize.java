...
public class ImageSize {
	...
	public static Dimension get(byte[] bytes) {
		return null;
	}
	...
	private static Dimension getSizeUsingImageIO(File file) throws IOException {
	...
	public static Dimension getSizeUsingImageIO(byte[] bytes) throws IOException {
		ImageInputStream iis = null;
		ImageReader reader = null;
		try {
			iis = new ImageInputStreamImpl() {
				@Override
				...
				public int read(byte[] b, int off, int len) throws IOException {
					int end = (int)streamPos + len;
					int newLen = len;
					if (end > bytes.length) {
						newLen = bytes.length - (int)streamPos;
						if (newLen < 0) {
							return -1;
						}
					}
					System.arraycopy(bytes, (int)streamPos, b, off, newLen);
					streamPos += newLen;
					return newLen;
				}
			};
