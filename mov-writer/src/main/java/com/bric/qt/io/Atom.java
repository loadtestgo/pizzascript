...
abstract class Atom implements TreeNode {
	...
	protected static void read(InputStream in,byte[] array,int offset,int length) throws IOException {
		...
		if(lastRead==-1) {
			System.err.println("read "+totalRead+" bytes, although "+length+" were requested");
	...
	protected synchronized static int read16Int(InputStream in) throws IOException {
		read(in,array2);
	...
	protected synchronized static final void write8Int(OutputStream out,int i) throws IOException {
	...
	protected synchronized static final void write32Int(OutputStream out,long i) throws IOException {
	...
	protected synchronized static final void write32String(OutputStream out,String s) throws IOException {
		if(s.length()==0) {
			array4[0] = 0;
			array4[1] = 0;
			array4[2] = 0;
